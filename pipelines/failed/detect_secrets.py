...
class Pipeline(PipelineBase):
    ...
    async def inlet(self, body: Dict[str, Any], __user__: Optional[dict] = None, dictExtParameter:dict = None, dictOuputResponse:dict = None, __request__: Optional[Request] = None) -> Dict[str, Any]:
        ...
        for msg in messages:
            ...
            if std_action == "block":
                policy_name = dictDetectedRule.get("name", "알 수 없는 정책")
                pii_types = os_doc_final["pii"]["types"]
    
                # 차단 메시지 구성
                block_message = f"""🚫 보안 정책: {policy_name}

입력하신 메시지를 처리할 수 없습니다.
민감정보를 제거한 후 다시 시도해 주세요."""
                )
#                block_message = (
#                    f"🚫 보안 정책 위반 감지\n\n"
#                    f"차단 사유: {policy_name}\n"
#                    f"입력하신 메시지에 민감정보가 포함되어 있어 처리할 수 없습니다.\n"
#                    f"개인정보 및 기밀 정보를 제거한 후 다시 시도해 주세요."
#                )
                raise Exception(block_message)
#                raise Exception("⚠️ 보안 정책: 민감정보가 포함된 메시지는 처리할 수 없습니다. 개인정보 보호를 민감정보를 제거해 주세요.")
