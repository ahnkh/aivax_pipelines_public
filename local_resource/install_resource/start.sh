...
function install_frontmatter_requirements() {
  local file=$1
...
function main()
{
  ...
  #TODO: 복잡하게 하지 말고, requirements.txt를 pip로 설치후, uvicorn을 실행한다.
  #실행은 venv 환경에서 실행한다.

  #/usr/local/bin/python /usr/local/bin/uvicorn main:app --host 0.0.0.0 --port 9099 --forwarded-allow-ips * --loop auto
...
# 다음 명령, 직접 호출
# TODO: 프로세스 실행관리 - 데몬 기동, 포트 관리 기능 필요 (guard)
