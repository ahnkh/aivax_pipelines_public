...
class _QuickPII:
    ...
    def hits_with_values(cls, text: Any, check_card_luhn: bool = True, max_per_type: int = 5) -> Dict[str, List[str]]:
        ...
        for m in cls.MILITARY_ID.finditer(text): add("military_id", m.group(0))
...
# 마스킹 유틸 (로그용: 가독 마스킹)
# ----------------------------
def _mask_email(_: str) -> str: return "[이메일]"
def _mask_phone(_: str) -> str: return "[전화번호]"
def _mask_credit_card_preserve_seps(_: str) -> str: return "[신용카드번호]"
def _mask_rrn(_: str) -> str: return "[주민등록번호]"
def _mask_passport(_: str) -> str: return "[여권번호]"
def _mask_ip(_: str) -> str: return "[IP주소]"
def _mask_generic(_: str) -> str: return "[개인정보]"
def _mask_driver_license(_: str) -> str: return "[운전면허번호]"
def _mask_foreign_rrn(_: str) -> str: return "[외국인등록번호]"
def _mask_business_reg(_: str) -> str: return "[사업자등록번호]"
def _mask_health_insurance(_: str) -> str: return "[건강보험번호]"
def _mask_tax_info(_: str) -> str: return "[세금정보]"
def _mask_address(_: str) -> str: return "[주소]"
...
class Pipeline:
    def __init__(self):
        ...
        class Valves(BaseModel):
            ...
            # ── 정책 드롭다운(문자열 설정) ──
            on_block_policy: str = Field(
                "masking",
                description='처리 정책: "masking" | "block" | "allow"',
                json_schema_extra={"enum": ["block", "masking", "allow"]},
            )
            on_mask_response_mode: str = Field(
                "redact",
                description='UI 안내문 형식: "redact" | "message_only" | "message_with_mask_info"',
            )
            ...
            # ── 추가 패턴 ──
            extra_patterns: List[Union[str, Any]] = Field(
                default_factory=list,
                description="Custom regex patterns (str or compiled)"
            )
    ...
    def _enabled_types(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "military_id": v.enable_military_id,
        }
    ...
    def _patterns_for_selected(self) -> List[re.Pattern]:
        ...
        if flags.get("military_id"): pats.append(_QuickPII.MILITARY_ID)
